...
pub inline fn parseDeclarationStmt(declaration_desc: ?lexer.Keyword, allocator: std.mem.Allocator, tokens: ?[]const lexer.Token, index: usize) ParseResult(ast.Statement) {
    ...
    if (_expect(TokenCategory, tokens.?[i], .id)) {
        ...
        if (_expect(lexer.BluntSymbol, tokens.?[i], .bind_)){
            ...
            const expr_node = try parseExpr(allocator, tokens, i);
            expression.* = expr_node.node;
            if (_expect(lexer.BluntSymbol, tokens.?[expr_node.end], .semi_colon_)){
...
pub fn parseFunctionCallPrime(allocator: std.mem.Allocator, tokens: ?[]const lexer.Token, index: usize) ParseResult(ast.Expr) {
    var i = index;
    const N = tokens.?.len;
